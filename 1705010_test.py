...
class TestDataLoader:
    ...
    def load(self):
        ...
        for image_name in image_names:
            image = cv2.imread(self.image_path + "/" + image_name, cv2.IMREAD_COLOR)
            images.append(image)
...
def predict(X_test, image_names):
    ...
    ## save predictions in csv with image names
    """
    FileName,Digit
    """
    df = pd.DataFrame()
    ...
    df['Digit'] = y_pred
    df.to_csv("1705010_prediction.csv", index=False)
...
if __name__ == '__main__':
    ...
    start = time.time()
    # testdataloader = TestDataLoader("dataset/NumtaDB_with_aug/sample-test")
